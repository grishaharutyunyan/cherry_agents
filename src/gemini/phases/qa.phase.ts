...
import { makeLintHardcodedColorsTool } from '../tools/lint-hardcoded-colors.tool';
...
You have six tools: read_file, list_files (whole monorepo, read-only), run_shell_backend and run_shell_frontend (npm run lint / npm run build only, cwd pinned to game_backend / game-frontend respectively — not a general shell), run_simulation (writes and runs a Monte Carlo script against game_backend's real code via ts-node, then deletes it — your only way to execute code), lint_hardcoded_colors (deterministic — call it once if a design-tokens JSON exists for this game, don't try to eyeball this check yourself), and report_qa_result (call this exactly once, as your final action, with your full findings — do not also reply with a plain-text final answer).
...
function buildUserMessage(parsedFields: AiAgentRunParsedFields, specDocContent: string, designTokensContent: string | null): string {
  const tokens = designTokensContent
    ? `

Design tokens JSON (run lint_hardcoded_colors and check tier wiring against this — see §4 of the checklist):

---
${designTokensContent}
---`
    : '\n\nNo design-tokens JSON exists for this game — skip §4 of the checklist.';
...
export async function runQaPhase(
  ...
  const gameFrontendDir = path.join(config.gameFrontendPath, 'games', parsedFields.gameId);
  ...
  const result = await runAgenticSession({
    ...
    tools: [
      ...
      makeLintHardcodedColorsTool(config.monorepoRoot, gameFrontendDir, []),
    ...
    initialUserMessage: buildUserMessage(parsedFields, specDocContent, designTokensContent),
